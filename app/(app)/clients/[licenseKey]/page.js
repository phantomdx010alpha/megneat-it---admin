 ...
 *
 * ── ADMIN_STATIC.md (device-delete/project-edit track), Phase 2 ─────────
 * Adds a delete action per device card, calling actions.js's own
 * `deleteDeviceAction` (Phase 1). Gated behind a type-to-confirm
 * BottomSheet — the same friction bar the client-delete flow on
 * `app/(app)/clients/page.js` already established, not a single click.
 * The confirm dialog names the specific device (its own name, or its raw
 * device id for the unnamed case — device names aren't guaranteed to be
 * set) plus its last-seen time, so the operator can't confuse two similar
 * device rows before deleting one. The required confirm text is the
 * device's own name when it has one, falling back to its device id when
 * it doesn't (a device name can be null; a device id never is).
 *
 * The dialog copy is deliberately honest about what this does and doesn't
 * do, per this masterplan's own Background note: it removes the
 * registration row only. It does not, and cannot from this repo, stop a
 * shell that's already running from continuing to sync — shell-side
 * enforcement of a deleted device is separate, ongoing work outside this
 * track. Nothing here claims otherwise.
 *
 * On success, the deleted device is filtered out of local state directly
 * (no full reload) — same pattern `confirmDelete` on the clients list page
 * already uses after `deleteClientAction` succeeds.
 */
...
import {
  ...
  listMoveTargetProjectOptionsAction,
  moveClientAction,
  deleteDeviceAction,
} from './actions';
...
export default function ClientDetailPage() {
  ...
  const [deleteDeviceDialog, setDeleteDeviceDialog] = useState(null); // { device, confirmText, deleting, error } | null
  ...
  function openDeleteDeviceDialog(device) {
    setDeleteDeviceDialog({ device, confirmText: '', deleting: false, error: null });
  }
  ...
  async function confirmDeleteDevice() {
    if (!deleteDeviceDialog || !detail) return;
    const { device } = deleteDeviceDialog;
    setDeleteDeviceDialog((d) => ({ ...d, deleting: true, error: null }));
    try {
      const token = await withToken();
      await deleteDeviceAction(token, {
        licenseKey: detail.licenseKey,
        deviceId: device.deviceId,
        projectId: detail.projectId,
      });
      setDetail((d) => (d ? { ...d, devices: d.devices.filter((dv) => dv.deviceId !== device.deviceId) } : d));
      setDeleteDeviceDialog(null);
    } catch (err) {
      setDeleteDeviceDialog((d) => ({ ...d, deleting: false, error: err.message }));
    }
  }
  ...
  return (
    ...
    >
                  ...
                  <Card key={device.id} padding="md">
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)', alignItems: 'flex-end', flexShrink: 0 }}>
                        ...
                        <Button variant="ghost" size="sm" onClick={() => openDeleteDeviceDialog(device)}>
                          <Trash size={14} weight="bold" aria-hidden="true" color="var(--color-danger)" />
                        </Button>
                      </div>
                    </div>
                  </Card>
      ...
      <BottomSheet
        open={Boolean(deleteDeviceDialog)}
        onClose={deleteDeviceDialog?.deleting ? undefined : closeDeleteDeviceDialog}
        title="Delete device"
      >
        {deleteDeviceDialog && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
            <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--text-secondary)', margin: 0 }}>
              This permanently removes <strong>{deleteDeviceDialog.device.deviceName || deleteDeviceDialog.device.deviceId}</strong>&apos;s
              registration ({formatDateTime(deleteDeviceDialog.device.lastSeenAt)} last seen). This cannot be undone.
            </p>
            ...
            >
              <Warning size={16} weight="bold" aria-hidden="true" style={{ flexShrink: 0, marginTop: 2 }} />
              If this is a PWA, that install will need to reactivate. If it's a shell, nothing here currently
              stops it from continuing to sync — shell-side enforcement of a deleted device is separate,
              ongoing work.
            </p>

            <Input
              id="delete-device-confirm"
              label={`Type "${deleteDeviceDialog.device.deviceName || deleteDeviceDialog.device.deviceId}" to confirm`}
              value={deleteDeviceDialog.confirmText}
              onChange={(e) => setDeleteDeviceDialog((d) => ({ ...d, confirmText: e.target.value }))}
              disabled={deleteDeviceDialog.deleting}
            />

            {deleteDeviceDialog.error && (
              <p role="alert" style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-danger)', margin: 0 }}>
                {deleteDeviceDialog.error}
              </p>
            )}
            ...
            <div style={{ display: 'flex', gap: 'var(--space-3)' }}>
              <Button
                ...
                disabled={
                  deleteDeviceDialog.deleting ||
                  deleteDeviceDialog.confirmText !==
                    (deleteDeviceDialog.device.deviceName || deleteDeviceDialog.device.deviceId)
                }
                fullWidth
              >
                <Trash size={16} weight="bold" aria-hidden="true" />
                {deleteDeviceDialog.deleting ? 'Deleting...' : 'Delete permanently'}
              </Button>
              <Button variant="ghost" onClick={closeDeleteDeviceDialog} disabled={deleteDeviceDialog.deleting}>
                Cancel
              </Button>
            </div>
          </div>
