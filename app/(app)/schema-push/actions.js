 ...
 *
 * ── Phase 5 (ADMIN_PANEL_DEVICE_DELETE_AND_PROJECT_EDIT_MASTERPLAN.md):
 * history view, and a gap that surfaced while building it ──────────────
 * `listSchemaPushHistoryAction` below reads back exactly the
 * `schema_push_broadcast` audit entries this file already writes (the
 * action string was re-read directly from the `logAuditEvent()` call
 * above, not assumed — it does not match the masterplan's own guess of
 * `'schema_push'`). Confirmed by reading `details` as actually written:
 * it holds `project_ids`, `succeeded_count`, `failed_count`,
 * `folded_into_master`, and per-project `{ projectId, label, status }` —
 * it does **not** hold the submitted SQL text, and a failed project's
 * `error` string (held in this file's own in-memory `results`, e.g. line
 * ~275) is never written to the log either, only its `status`. The
 * masterplan's own Phase 5 text describes the history view as including
 * "a preview/expandable view of the SQL submitted" on the assumption
 * that's "already captured in that log entry's own details field" — it
 * isn't, for either the SQL or a failure's error text. Adding either one
 * now would be a new logging mechanism, which Phase 5's own text rules
 * out ("Out of scope: Any new logging mechanism — this phase only
 * surfaces what Phase 4 of the original schema-broadcast track already
 * records"). So the history view below surfaces exactly what's on each
 * row — when, actor, counts, per-project status, fold outcome — and says
 * plainly that the raw SQL and per-project error text aren't available
 * from history (only during the run itself, in the results card above).
 * A future phase could add `sql` to this file's own `logAuditEvent` call
 * if that's ever wanted; not decided here, not silently done here either.
 *
...
const SCHEMA_PUSH_HISTORY_ROW_LIMIT = 100;

/**
 * Phase 5 (ADMIN_PANEL_DEVICE_DELETE_AND_PROJECT_EDIT_MASTERPLAN.md) —
 * every past `schema_push_broadcast` audit entry, most-recent-first, for
 * this screen's own dedicated history view. Same "single capped query, no
 * pagination machinery" reasoning as `listAuditLogAction`
 * (app/(app)/activity/actions.js) — this is a single-operator tool with a
 * modest, occasional-action volume, not a table that needs real pagination
 * yet.
 *
 * See this file's own top comment for what `details` does and doesn't
 * hold — notably, no SQL text and no per-project error text, only the
 * outcome. That's surfaced here as-is, not silently reshaped to look more
 * complete than it is.
 *
 * @param {string} accessToken
 ...
 *   failedCount: number,
 *   foldedIntoMaster: boolean,
 *   results: Array<{ projectId: string, label: string | null, status: 'success' | 'failed' }>,
 * }>>}
 */
export async function listSchemaPushHistoryAction(accessToken) {
  ...
  const { data, error } = await admin
    .from('audit_log')
    .select('id, actor, target, details, created_at')
    .eq('action', 'schema_push_broadcast')
    .order('created_at', { ascending: false })
    .limit(SCHEMA_PUSH_HISTORY_ROW_LIMIT);

  if (error) {
    throw new Error(`Could not load schema push history: ${error.message}`);
  }
  ...
  return (data ?? []).map((row) => {
    ...
    return {
      ...
      projectIds: details.project_ids ?? [],
      succeededCount: details.succeeded_count ?? 0,
      failedCount: details.failed_count ?? 0,
      foldedIntoMaster: details.folded_into_master ?? false,
